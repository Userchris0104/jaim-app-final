...
interface Ad {
  ...
  // Text overlay data (rendered client-side)
  textOverlay?: {
    brandName: string;
    headline: string;
    cta: string;
...
// Text Overlay component - renders brand, headline, and CTA on top of ad images
function TextOverlay({
  overlay,
  showOverlay = true
}: {
  overlay: Ad['textOverlay'];
  showOverlay?: boolean;
}) {
  if (!overlay || !showOverlay) return null;

  const isDark = overlay.theme === 'light-on-dark' || overlay.theme === 'auto';
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-violet-200 transition-all group">
      ...
      <div className="relative aspect-[4/5] bg-gray-100 overflow-hidden">
        ...
        {ad.textOverlay && (
          <TextOverlay overlay={ad.textOverlay} />
        )}
